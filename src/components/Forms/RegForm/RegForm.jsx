  ...
  return (<>
    ...
    <Formik {...props}
      ...
      validationSchema={yup.object({
        ...
        email: yup.string().email(errors.email).notOneOf(usedEmails, errors.usedEmail).required(errors.required),
        phone: yup.string().matches(phoneRegExp, errors.phone).required(errors.required),
        ...
        confirmPass: yup.string().when("password", {is: (password) => (password && password.length > 0 ? true : false),then: yup.string().oneOf([yup.ref("password")], errors.confirmPass),}).required(errors.required),})}
